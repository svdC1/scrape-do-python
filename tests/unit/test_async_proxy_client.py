...
class TestAsyncProxyClientInitialization:
    ...
    async def test_api_token_from_env(
        self,
    ...
    async def test_async_context_manager_exit_returns_false(
        ...
        """
        client = AsyncScrapeDoProxyClient(api_token="test")
        result = await client.__aexit__(None, None, None)
        assert result is False


class TestAsyncProxyClientPool:
    """Behavioral tests for the async LRU client pool."""

    @respx.mock
    async def test_pool_caches_client_per_proxy_url(
        self,
        ...
        await mock_async_proxy_client.execute(req1)
        await mock_async_proxy_client.execute(req2)

        assert len(mock_async_proxy_client._client_pool) == 1

    @respx.mock
    async def test_pool_partitions_by_proxy_url(
        self,
        mock_async_proxy_client: AsyncScrapeDoProxyClient,
        make_request,
        mock_env_vars,
        ...
        assert len(mock_async_proxy_client._client_pool) == 2
    ...
    async def test_pool_evicts_lru_when_full(
        ...
        """
        Ensures the LRU entry is evicted (and aclosed) when the pool
        ...
        async with AsyncScrapeDoProxyClient(
            ...
            await client.execute(make_request(super=True))
            await client.execute(make_request(super=False))
            await client.execute(make_request(super=True, render=False))
    ...
    async def test_pool_lru_touch_on_hit(
        ...
        async with AsyncScrapeDoProxyClient(
            ...
            await client.execute(req_a)
            await client.execute(req_b)
            await client.execute(req_a)  # touch A
            await client.execute(req_c)  # evict B, not A
    ...
    async def test_aclose_drains_pool(
        ...
        """
        Ensures aclose() aclose-es every pooled `httpx.AsyncClient`
        and empties the pool.
        """
        respx.get("https://example.com").respond(status_code=200)

        client = AsyncScrapeDoProxyClient(api_token="test", verify=False)
        await client.execute(make_request(super=True))
        await client.execute(make_request(super=False))
    ...
    async def test_async_context_manager_drains_pool_on_exit(
        ...
        """
        Ensures exiting the async context manager closes every pooled
        `httpx.AsyncClient`.
        """
        respx.get("https://example.com").respond(status_code=200)
    ...
    async def test_concurrent_misses_on_same_url_dont_leak_clients(
        ...
        """
        Ensures the asyncio.Lock prevents two concurrent coroutines from
        creating redundant `httpx.AsyncClient` instances on the same
        cache-miss key.
        """
        respx.get("https://example.com").respond(status_code=200)
        ...
        async with AsyncScrapeDoProxyClient(
            ...
            # Two concurrent requests with identical params hit the same
            # proxy URL. With the lock, only one client is created.
            req1 = make_request()
            req2 = make_request()
            ...
            assert len(client._client_pool) == 1


class TestAsyncProxyClientRouting:
    ...
    async def test_request_param_constraints(
        ...
        """
        with patch.object(
            mock_async_proxy_client,
            "execute",
            new_callable=mocker.AsyncMock
        ):
            with pytest.raises(ValueError, match=error_match):
                await mock_async_proxy_client.request(**request_kwargs)
    ...
    async def test_get_routing(
        ...
        with patch.object(
            ...
            await mock_async_proxy_client.get(
                "https://example.com",
                render=False
                )
    ...
    async def test_post_routing(
        ...
        with patch.object(
            ...
            await mock_async_proxy_client.post(
                "https://example.com",
                render=False
                )
            args, kwargs = mock_request.call_args
...
class TestAsyncProxyClientExecutionEngine:
    ...
    async def test_successful_request(
        ...
        response = await mock_async_proxy_client.execute(req)
    ...
    async def test_retry_on_gateway_errors_success(
        ...
        """
        Ensures the client retries on Scrape.do gateway errors (429, 502)
    ...
    async def test_max_retries_exhausted_returns_error_response(
        ...
        """
        mock_async_proxy_client.retry_backoff = 10.0
        req = make_request()
    ...
    async def test_network_error_raises_api_connection_error(
        ...
        """
        mock_async_proxy_client.max_retries = 1
        req = make_request()
        route = respx.get("https://example.com")
        route.side_effect = [
...
class TestAsyncProxyClientSessionValidator:
    ...
    async def test_validator_true_raises_rotated_session_error(
        ...
        """
        Ensures the SDK raises RotatedSessionError when the async
        ...
        with pytest.raises(RotatedSessionError):
            await mock_async_proxy_client.execute(req, detect_logged_out)

    @respx.mock
    async def test_validator_false_returns_response(
        self,
        mock_async_proxy_client: AsyncScrapeDoProxyClient,
        make_request,
        mock_env_vars,
        ...
        """
        Ensures an async validator that returns False yields the
        response as normal.
        """
        req = make_request(session_id=10)
        respx.get("https://example.com").respond(status_code=200)
        ...
        response = await mock_async_proxy_client.execute(req, _always_false)
        assert response.scrape_do_status_code == 200
    ...
    async def test_validator_skipped_when_no_session_id(
        ...
        response = await mock_async_proxy_client.execute(req, validator)

        validator.assert_not_called()
        assert response.scrape_do_status_code == 200


class TestAsyncProxyClientEventHooks:
    ...
    async def test_request_hook_fires_once_regardless_of_retries(
        ...
        """
        Ensures the request hook is awaited exactly once even when
        retries occur underneath.
        """
        req = make_request()
        request_hook = mocker.AsyncMock()
        mock_async_proxy_client.event_hooks = {"request": [request_hook]}
    ...
    async def test_response_hook_fires_once_on_success(
        ...
        """
        Ensures the response hook is awaited exactly once on success.
        """
        req = make_request()
        response_hook = mocker.AsyncMock()
        mock_async_proxy_client.event_hooks = {"response": [response_hook]}
    ...
    async def test_retry_hook_fires_with_response_on_retryable_status(
        ...
        when retrying on a 429/502/510 status.
        """
        req = make_request()
        retry_hook = mocker.AsyncMock()
        mock_async_proxy_client.event_hooks = {"retry": [retry_hook]}
    ...
    async def test_no_event_hooks_default_does_not_break(
        ...
        """
        req = make_request()
        assert mock_async_proxy_client.event_hooks == {}
    ...
    @respx.mock
    async def test_retry_hook_fires_with_exception_on_network_error(
        self,
        mock_async_proxy_client: AsyncScrapeDoProxyClient,
        make_request,
        mocker,
        ...
        retry_hook.assert_awaited_once()
        attempt, hook_request, hook_response, hook_exc = (
            retry_hook.call_args[0]
            )
        assert attempt == 0
...
class TestAsyncProxyClientRequestOverrides:
    """
    ...
    async def test_callable_backoff_on_network_error(
        ...
        number on the RequestError path.
        """
        backoff_calls = []
    ...
    @respx.mock
    async def test_request_level_timeout_override(
        self,
        mock_async_proxy_client: AsyncScrapeDoProxyClient,
        make_request,
        mocker,
        ...
        await mock_async_proxy_client.execute(req, r_timeout=custom_timeout)

        spy.assert_called_once()
        assert spy.call_args.kwargs.get("timeout") == custom_timeout

    @respx.mock
    async def test_request_level_extensions_override(
        self,
        mock_async_proxy_client: AsyncScrapeDoProxyClient,
        make_request,
        mocker,
        ...
        await mock_async_proxy_client.execute(req, extensions=extensions)
